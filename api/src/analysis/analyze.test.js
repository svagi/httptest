...
domains.forEach((domain) => datasets[domain].forEach((har, idx) => {
  ...
  test(`[${idx}|${domain}] page.totalRequests is number greater or equal to zero`, (t) => {
    t.true(analysis.page.totalRequests >= 0)
  })

  test(`[${idx}|${domain}] page.totalRedirects is number greater or equal to zero`, (t) => {
    t.true(analysis.page.totalRedirects >= 0)
  })

  test(`[${idx}|${domain}] page.http2Requests is number greater or equal to zero`, (t) => {
    t.true(analysis.page.http2Requests >= 0)
  })

  test(`[${idx}|${domain}] page.totalBytes is number greater or equal to zero`, (t) => {
    t.is(typeof analysis, 'object')
  })

  test(`[${idx}|${domain}] page.domLoadTime is number greater or equal to zero or -1`, (t) => {
    t.true(analysis.page.domLoadTime >= 0)
  })

  test(`[${idx}|${domain}] page.loadTime is number greater or equal to zero or -1`, (t) => {
